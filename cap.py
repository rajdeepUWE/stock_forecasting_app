...
def main():
    ...
    selected_model = st.selectbox('Select Model', ['Keras Neural Network', 'Support Vector Regressor (SVR)', 'LSTM'])
    ...
    if selected_model == 'Keras Neural Network':
        ...
        y_true = data['Close'].values[-7:]  # Take the last 7 days' true values
        X_pred = np.arange(len(data), len(data) + 7).reshape(-1, 1)
        y_pred = forecast_next_7_days_svr(data, model, scaler)
    elif selected_model == 'LSTM':
        model = lstm_model
        scaler = lstm_scaler
        y_true = data['Close'].values[-7:]  # Take the last 7 days' true values
        y_pred = forecast_next_7_days_lstm(data, model, scaler)
